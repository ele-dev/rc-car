...
SteeringController::SteeringController(GPIO& gpio_ptr) : m_gpio(gpio_ptr)
{
...
{
    // this->m_gpio_handle = -1;
    std::cout << "[SteeringController] memory released\n";
}
...
bool SteeringController::init()
{
    ...
    if(set_mode(this->m_gpio.get_handle(), GPIO_SERVO, PI_OUTPUT) != 0) {
...
void SteeringController::update_steering_angle(const int angle) const
{
    ...
    set_servo_pulsewidth(this->m_gpio.get_handle(), GPIO_SERVO, pulse_duration);
}

void SteeringController::center_steering_servo() const
{
    set_servo_pulsewidth(this->m_gpio.get_handle(), GPIO_SERVO, CENTER_PULSE_DURATION);
}
